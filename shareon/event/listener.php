...
class listener implements EventSubscriberInterface
{
    ...
    public function common_setup($event)
    {
        $this->template->assign_vars(array(
            ...
            'S_SO_OK'			=> $this->config['so_ok'] ? true : false,
            'S_SO_MYMAIL'		=> $this->config['so_mymail'] ? true : false,
        ));
    }
    ...
    public function viewtopic_postrow_shareon($event)
    {
        ...
        $postrow = array_merge($postrow, array(
            'U_FACEBOOK'	=> 'https://facebook.com/sharer/sharer.php?u=' . urlencode($share_url),
            'U_TWITTER'		=> 'https://twitter.com/share?url=' . urlencode($share_url) . '&amp;text=' . urlencode($topic_title),
            'U_DIGG'		=> 'https://digg.com/submit?url=' . urlencode($share_url) . '&amp;title=' . urlencode($topic_title),
            'U_REDDIT'		=> 'https://reddit.com/submit?url=' . urlencode($share_url) . '&amp;title=' . urlencode($topic_title),
            'U_DELICIOUS' 	=> 'https://delicious.com/post?url=' . urlencode($share_url) . '&amp;title='. urlencode($topic_title),
            'U_VK'			=> 'https://vk.com/share.php?url=' . urlencode($share_url) . '&amp;title= ' . urlencode($topic_title),
            ...
            'U_TUMBLR'		=> 'https://tumblr.com/widgets/share/tool?canonicalUrl=' . urlencode($share_url) . '&amp;posttype=link&amp;title=' . urlencode($topic_title),
            'U_GOOGLE'		=> 'https://plus.google.com/share?url=' . urlencode($share_url),
            'U_OK'			=> 'https://connect.ok.ru/offer?url=' . urlencode($share_url),
            'U_MYMAIL'		=> 'https://connect.mail.ru/share?url=' . urlencode($share_url) . '&amp;title=' . urlencode($topic_title)
        ));
